## RSAmyMRI
[![Build Status](https://travis-ci.org/uwescience/RSAmyMRI.svg?branch=master)](https://travis-ci.org/uwescience/RSAmyMRI)
...
`RSAmyMRI/RSAmyMRI.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `RSAmyMRI.py` in directory called
`RSAmyMRI`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import RSAmyMRI as sb` in an
interactive Python session, the classes and functions defined inside of the
`RSAmyMRI.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the RSAmyMRI module, the main classes defined at the bottom of
...
Either way, you can create a `RSAmyMRI/data` folder in which you can
...
(typically, something like our `RSAmyMRI/tests/test_RSAmyMRI.py`). Within each
...
have one such test in `RSAmyMRI/tests/test_RSAmyMRI.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/RSAmyMRI`),
and type:
...
will see a message such as:
    ...
    RSAmyMRI/tests/test_RSAmyMRI.py .F...
    ...
    RSAmyMRI/tests/test_RSAmyMRI.py:49: AssertionError
...
To document `RSAmyMRI` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/RSAmyMRI/blob/master/doc/_static/RTD-advanced-conf.png)

 http://RSAmyMRI.readthedocs.org/en/latest/
...
A `RSAmyMRI/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/RSAmyMRI) for the software. This
...
`RSAmyMRI` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `RSAmyMRI`, we use the
...
[here](https://github.com/uwescience/RSAmyMRI/blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "RSAmyMRI" repository.
...
`RSAmyMRI/RSAmyMRI.py` to be called `smallish/smallish.py`

	git mv RSAmyMRI smallish
	git mv smallish/RSAmyMRI.py smallish/smallish.py
	git mv smallish/tests/test_RSAmyMRI.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `RSAmyMRI` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `RSAmyMRI` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `RSAmyMRI` in them. Type
the following to see where all these files are:

	git grep RSAmyMRI

You can replace `RSAmyMRI` for `smallish` quickly with:

	git grep -l 'RSAmyMRI' | xargs sed -i 's/RSAmyMRI/smallish/g'
...
Edit all the mentions of `RSAmyMRI` in the `RSAmyMRI/__init__.py` file, and in
the `RSAmyMRI/version.py` file as well.
